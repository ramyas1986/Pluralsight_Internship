...
const AddAuthorForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await addAuthor({ name, biography, authorImage: imageFile });
      const authorId = response.data.authorId;
      toast.success('Author added successfully!');
      
      if (fromPage === 'add-book') {
        navigate('/add-book', { state: { autoPopulateAuthorId: authorId } });
      } else if (fromPage === 'authors') {
        navigate('/authors');
      } else {
        navigate('/authors'); // Default fallback
      }
  ...
  const handleCancel = () => {
    navigate(fromPage === 'add-book' ? '/add-book' : '/authors');
  };
