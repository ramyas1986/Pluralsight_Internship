...
const EditBookForm = ({ book, onClose }) => {
  ...
  const [publicationDate, setPublicationDate] = useState(new Date(book.publicationDate));
  ...
  const handleSubmit = async (e) => {
    ...
    // Convert publication date to UTC and then to ISO string
    const updatedBookData = {
      ...formData,
      publicationDate: new Date(publicationDate.getTime() - publicationDate.getTimezoneOffset() * 60000)
        .toISOString()
        .split('T')[0],
