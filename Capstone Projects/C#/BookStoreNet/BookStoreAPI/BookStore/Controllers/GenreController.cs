...
public class GenresController : ControllerBase
{
    ...
    public async Task<IActionResult> AddGenre([FromBody] Genre genre)
    {
        ...
        if (genre.GenreName.Contains("!"))
        {
            throw new Exception("Genre cannot contain special characters");
        }

        try
        {
            var existingGenere = _context.Genres.FirstOrDefault(a=>a.GenreName == genre.GenreName);
            if (existingGenere != null)
            {
                return BadRequest(new { error = "Genre name already exists" });
            }
